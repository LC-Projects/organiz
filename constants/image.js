const logo = {
  organiz: require("../assets/logo/logo_organiz.png"),
  google: require("../assets/logo/google.png"),
};

const auth = {
  top: require("../assets/imgs/auth/top_left.png"),
  intro: require("../assets/imgs/auth/intro.png"),
};

const icon = {
  plus: require("../assets/icons/icon-plus.png"),
};

const json = {
  homepage: require("../assets/json/homepage-welcome-animation.json"),
  login: require("../assets/json/login-animation.json"),
  register: require("../assets/json/register-animation.json"),
  profile: require("../assets/json/profile-animation.json"),
}

export default {
  logo,
  auth,
  icon,
  json
};
